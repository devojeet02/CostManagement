/**
 * SCENARIO MANAGEMENT - PREVIOUS VERSION (read-only reference).
 *
 * A verbatim snapshot of the screen as it stood before the CCM-047/048/049/051 port, kept
 * routable at /scenario-management-legacy purely so the two can be compared side by side.
 *
 * What it does NOT have, and what the comparison is about:
 *   - no Columns picker: every scenario column is always on screen
 *   - no variance BUILDER: the two comparison columns are auto-derived and cannot be changed
 *   - no saved layout and no Reset to Default - nothing survives leaving the screen
 *   - no Cumulative Variance row
 *
 * Nothing here is maintained. It reads the same mock service as the live screen, so the figures
 * match and only the UI differs.
 */
...
@Component({
  selector: 'app-scenario-management-legacy',
  templateUrl: './scenario-management-legacy.component.html',
  styleUrls: ['./scenario-management-legacy.component.scss']
})
export class ScenarioManagementLegacyComponent implements OnInit, AfterViewInit, OnDestroy {
   ...
   */
  scenarioTypeGroups: SelectGroup[] = ScenarioManagementLegacyComponent.buildTypeGroups([]);
  ...
  private loadScenarios(): void {
    this.masterDataService.getScenarios().subscribe({
      next: (rows: LookupItemDto[]) => {
        ...
        this.scenarioTypeGroups = ScenarioManagementLegacyComponent.buildTypeGroups(this.scenarios);
        this.loadCostRows();
      },
      error: err => {
  ...
  private static buildTypeGroups(scenarios: Scenario[]): SelectGroup[] {
    ...
    ScenarioManagementLegacyComponent.BASE_SCENARIO_TYPES.forEach(t => byKey.set(t.toLowerCase(), t));
    scenarios.forEach(s => {
